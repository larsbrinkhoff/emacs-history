...
gfnames(pac, pav, p_error)
	...
	tbool	*p_error;
{
	static	char	filename[MAX_FILE_SPEC_LEN];
		short	fn_exp();
	...
	char	*in;
{
	...
	if (pass1)
		...
		o.dsc$b_dtype = i.dsc$b_dtype = DSC$K_DTYPE_T;
		o.dsc$b_class = i.dsc$b_class = DSC$K_CLASS_S;
		}
...
/* v1.00 nmm 29-apr-85 trim_dsc - remove string descriptor trailing spaces */
/* note that pd->dsc$w_length must be at least 1 */
	...
	struct dsc$descriptor_s *pd;
{
	...
	for (s = pd->dsc$a_pointer + pd->dsc$w_length - 1, i = pd->dsc$w_length;
	     i && *s == ' '; s--, i--)
		;
	*(s + 1) = '\0';	
}
...
/* is_stream_file - simplistic check that a file which has just been
   opened is a VMS stream  file.  It assumes that if the file isn't stream,
   it won't be processed further, so it doesn't bother about leaving the
   pointer correctly positioned at start-of-file again
*/
is_stream_file(f)
     FILE *f;
{
