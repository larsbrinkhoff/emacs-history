...
#ifdef VMS
Lisp_Object Vmb_local_completion_map;
#else
...
#ifdef VMS
Lisp_Object Vmb_local_must_match_map;
#else
...
#ifdef VMS
  internal_output_to_temp_buffer ("*Help*", describe_buffer_bindings, thisbuf);
#else
     ...
     Lisp_Object string, pred, noprint;
{
  ...
  if (NULL (noprint))
#ifdef VMS
    internal_output_to_temp_buffer ("*Help*", apropos1,
#else
...
#ifdef VMS
  DefLispVar ("minibuffer-local-completion-map", &Vmb_local_completion_map,
#else
...
#ifdef VMS
  Vmb_local_completion_map = Fmake_sparse_keymap ();
#else
  Vminibuffer_local_completion_map = Fmake_sparse_keymap ();
#endif

#ifdef VMS
  DefLispVar ("minibuffer-local-must-match-map", &Vmb_local_must_match_map,
#else
...
#ifdef VMS
  Vmb_local_must_match_map = Fmake_sparse_keymap ();
#else
