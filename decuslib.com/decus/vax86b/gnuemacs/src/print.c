...
#ifdef VMS
      if ( vmsprint )
        vmschars += size >= 0 ? size : strlen( ptr );
#endif
      return;
    }
...
Lisp_Object
#ifdef VMS
internal_output_to_temp_buffer (bufname, function, args)
#else
...

#ifdef VMS
DEFUN ("vmsprint", Fvmsprint, Svmsprint, 1, 2, 0,
  "Just like print, but inserts newlines before line length exceeds\n\
a fixed number.  This is to avoid writing a file with record sizes\n\
exceeding 512 bytes, which causes problems in reading the file back.")
  (obj, printcharfun)
     Lisp_Object obj, printcharfun;
{
  struct buffer *old = bf_cur;
...
#ifdef VMS
  if ( vmsprint && vmschars > MAXVMSCHARS ) {
        PRINTCHAR ('\n');
	vmschars = 0;
        }
#endif
