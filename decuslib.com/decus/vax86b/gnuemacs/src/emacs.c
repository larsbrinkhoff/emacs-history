     ...
     int sig;
{
   ...
   lib$stop( SS$_ABORT );	/* Generate traceback */
#else
#ifdef BSD
  int tpgrp;
#endif /* BSD */
...
#endif
{
...
#ifdef VMS
  Lisp_Object tem;
  char arg_area[ 200 ], * argptr, * orig, * argv[ 20 ];
  int i;

  /* Copy argv's into local stack.
   */
  argptr = arg_area;
  ...
  for ( i = 0; i < argc; i++ ) {
    argv[ i ] = argptr;
    orig = orig_args[ i ];
    while (( argptr < &arg_area[ sizeof( arg_area ) - 1 ] ) &&
	   ( *argptr++ = *orig++ ))
      ;
    *argptr = '\0';
    }

  /* Unless the arguments specify otherwise, map the data area in.
   */
  process_vms_args( argc, argv );

  /* Fix up stdin, stdout, stderr if linked with shareable image.
   */
  if ( stdin == 0 )
    stdin = fdopen( 0, "r" );
  if ( stdout == 0 )
...
#ifdef VMS
  if (!noninteractive)
    init_vms_input ();	/* Assign terminal channel.  Must precede init_display */
#endif
  if (!noninteractive)
...
/* process_vms_args searches argument list looking for a load and dump
 * command.  If not found, it maps in the pure data file, and sets initialized
 * flag.
 */

int process_vms_args( argc, argv )
int
    ...
    ** argv;		/* Argument list */
{
    ...
    /* No load&dump arguments found.  Map data and alloc area in, if possible.
     */
    return ( mapin_data());
} /* process_vms_args */
...
#endif
#ifdef VMS
  exit ((XTYPE (arg) == Lisp_Int) ? XINT (arg) : 1);
#else
...

#ifdef VMS
DEFUN ("dump-emacs", Fdump_emacs, Sdump_emacs, 0, 0, 0,
  "Maps out the data into a map file")
  ()
{
  Lisp_Object temp;
