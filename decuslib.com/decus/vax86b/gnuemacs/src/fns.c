...
DEFUN ("random", Frandom, S_random, 0, 1, 0,
...
#ifdef VMS
DEFUN ("string-equal-nocase", Fstring_equal_nocase, Sstring_equal_nocase,
       2, 2, 0,
  "T if two strings have identical contents.\n\
Symbols are also allowed; their print names are used instead.\n\
Ignores case.")
     ...
     register Lisp_Object s1, s2;
{
  ...
  if (XSTRING (s1)->size != XSTRING (s2)->size ||
      ignorecasecmp (XSTRING (s1)->data, XSTRING (s2)->data,
		     XSTRING (s1)->size))
    return Qnil;
  return Qt;
}
