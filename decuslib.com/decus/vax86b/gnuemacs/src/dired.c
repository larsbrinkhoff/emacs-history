  ...
  for (passcount = !!all_flag; NULL (bestmatch) && passcount < 2; passcount++)
      ...
      while (dp = readdir (d))
	{
	  ...
	  if (!dp->d_ino ||
	    ...
	    continue;
#else
	  if (dp->d_namlen < XSTRING (file)->size ||
	      ignorecasecmp (dp->d_name, XSTRING (file)->data, XSTRING (file)->size))
	    continue;
#endif
	  if (!passcount)
...
#ifdef VMS
		if ( ignorecasecmp( dp->d_name + skip,
			  XSTRING (elt)->data,
			  XSTRING (elt)->size))
		  continue;
#else
...
#ifdef VMS
  temp = Fdirectory_file_name( filename );
  if ( ! NULL( temp ))
    filename = temp;
#endif
