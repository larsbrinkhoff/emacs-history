...
#ifdef VMS
static char user_real_name[17];	/* login ID of current user */
#else
...
#else /* IF VMS */
    user_name = (char *) getenv ("USER");
    strcpy( user_real_name, user_name );
    strcpy( user_full_name, user_name );
    strcpy( system_name, "VAX-VMS" );
#endif
}

...
DEFPRED ("bobp", Fbobp, Sbobp, "Return T if point is at the beginning of the buffer.",
	 point<=FirstCharacter)
DEFPRED ("eobp", Feobp, Seobp, "Return T if point is at the end of the buffer.",
	 point>NumCharacters)
DEFPRED ("bolp", Fbolp, Sbolp, "Return T if point is at the beginning of a line.",
	 point<=FirstCharacter || CharAt(point-1)=='\n')
DEFPRED ("eolp", Feolp, Seolp, "Return T if point is at the end of a line.",
