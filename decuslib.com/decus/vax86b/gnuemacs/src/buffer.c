...
#else
  /* On VMS, filenames are case insensitive.  Making buffer names
   * case sensitive turns out to be very user unfriendly.
   */
  for ( tem = Vbuffer_alist; !NULL( tem ); tem = Fcdr( tem )) {
    if ( ! NULL( Fstring_equal_nocase( name, Fcar( Fcar( tem )))))
	return ( Fcdr( Fcar( tem )));
    }
  return Qnil;
#endif
}
...
#ifdef VMS
      tem = Fstring_equal_nocase (XBUFFER (buf)->filename, filename);
#else
...
{
#ifdef VMS
  internal_output_to_temp_buffer ("*Buffer List*",
#else
