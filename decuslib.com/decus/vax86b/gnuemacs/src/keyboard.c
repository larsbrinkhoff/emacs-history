...
extern char input_buf_start[], * input_buf_end,
            * input_next_in, * input_next_out;
extern int input_ef, input_eflist, input_ast_wait;
extern int input_buffer_full;
extern int start_kbd_input();
extern int have_process_input, process_exited;
#define	PROC_CHECK	{ int dsp = have_process_input || process_exited;\
			  if ( have_process_input ) process_command_input();\
                          if ( process_exited ) process_exit();\
			  if ( dsp ) DoForceDsp(1); }
#endif VMS
...
#ifdef VMS
	internal_output_to_temp_buffer ("*Help*", print_help, tem);
#else
...
{
#ifdef VMS
  if ( input_next_in != input_next_out )
    *addr = 1;
  else
    *addr = 0;
#else
...
read_avail_input ()
{
  ...
  ioctl (fileno (stdin), TCSETA, &told);
    ...
    for (;;) {
      sys$setast( 0 );
      if ( input_next_out == input_next_in ) {
	sys$clref( input_ef );
	input_ast_wait = 1;
	sys$setast( 1 );
	...
	input_ast_wait = 0;
	if ( input_next_out == input_next_in )
	    PROC_CHECK;	/* Check for subprocess input availability */
	}
      else {
	sys$setast( 1 );
	break;
    ...
    while ( input_next_out != input_next_in ) {
	*ptr++ = *input_next_out++;
	kbd_count++;
	if ( input_next_out == input_buf_end )
	    input_next_out = &input_buf_start[0];
	}
    kbd_ptr = kbd_buffer;
...
{
#ifdef VMS
  error( "Suspending EMACS not provided on VMS." );
#else
#ifdef SIGTSTP			/* Support possible in later USG versions */
  if (!NULL (stuffstring))
