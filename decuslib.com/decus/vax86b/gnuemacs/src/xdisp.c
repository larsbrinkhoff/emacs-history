...

static char buf[MScreenWidth + 1];
...

#ifdef VMS
/* Call DoDsp, forcing it to look at all the windows.
 */
DoForceDsp (SaveMiniBuf)
{
    force_all_windows = 1;
    DoDsp (SaveMiniBuf);
    force_all_windows = 0;
...
#ifdef VMS
  all_windows = force_all_windows || RedoModes || buffer_shared > 1;
#else
