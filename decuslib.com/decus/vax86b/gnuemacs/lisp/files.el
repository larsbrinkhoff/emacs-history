;; File input and output commands for Emacs, VMS version
...
(defconst find-file-run-dired nil
...
(defun cd (dir)
  ...
  (if (not (file-directory-p (directory-file-name dir)))
...
Sets buffer mode, parses local variables, calls  find-file-hook."
  ...
  (if noninteractive
    ...
    (message (cond ((not buffer-read-only)
		   ...
		   (t
		    "File not found and not writeable")))
...
(defun basic-save-buffer ()
  ...
  (if (buffer-modified-p)
      (let (setmodes tempsetmodes sysprv bypass)
	(or buffer-file-name
   ...
   "Attempt to save to a file which you aren't allowed to write")))
	...
	(save-restriction
	  ...
	  (cond ((and tempsetmodes (not setmodes))
		 (or (setq sysprv (try-privilege "SYSPRV" buffer-file-name))
		     (progn (set-file-modes buffer-file-name 511 t)
			    (setq setmodes (file-modes buffer-file-name))
			    ;; Change the mode back, after writing.
			    (file-writable-p buffer-file-name))
		     (setq bypass (try-privilege "BYPASS" buffer-file-name)))))
	  (write-region (point-min) (point-max) buffer-file-name nil t)
	  (and sysprv (setprv "SYSPRV" nil))
	  (and bypass (setprv "BYPASS" nil))
	  (if setmodes
	      (condition-case ()
...
(defun try-privilege (priv file)
  (if (setprv priv nil t)
      nil			;; Privilege was already enabled.
      (if (setprv priv t)
          (if (and (file-writable-p file)
                   (y-or-n-p (format "Use %s? " priv)))
	      t
	      (setprv priv nil)
	      nil)
          nil)))
...
With argument, saves all with no questions."
  ...
  (let (considered (list (buffer-list)))
	...
	(condition-case ()
	    (save-excursion
		(set-buffer buffer)
		(or (and (buffer-modified-p buffer)
